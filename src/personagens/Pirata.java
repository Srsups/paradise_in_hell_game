...
public class Pirata extends Rectangle{
	...
	public Pirata(int x, int y,int screenWidth,int screenHeight) {
	    super(x, y, 32, 32);
	...
	public void tick(int screenWidth,int screenHeight) {
		...
		if(curFrames == targetFrames) {
			curFrames = 0;
			curAnimation++;
			if(curAnimation == Spritesheet.pirata_front.length) {
				curAnimation = 0;
			}
		}
...
public void render(Graphics g, int renderX, int renderY) {
		...
		if(down == true)
			g.drawImage(Spritesheet.pirata_front[curAnimation],x - Camera.x, y - Camera.y,32,32,null);
		
		else if(right == true)
			g.drawImage(Spritesheet.pirata_lado[curAnimation], x - Camera.x, y - Camera.y, 32, 32, null);
		
		else if(left == true)
			g.drawImage(Spritesheet.pirata_lado[curAnimation], x + 32 - Camera.x, y - Camera.y, -32, 32, null);
		
		else if(up == true)
			g.drawImage(Spritesheet.pirata_cima[curAnimation], x - Camera.x, y - Camera.y, 32, 32, null);
		
		else {
			g.drawImage(Spritesheet.pirata_front[0], x - Camera.x, y - Camera.y, 32 , 32, null);
		}
