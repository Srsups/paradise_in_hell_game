...
public class Mago extends Rectangle{
	...
	public Mago(int x, int y,int screenWidth,int screenHeight) {
	    super(x, y, 32, 32);
	...
	public void tick(int screenWidth,int screenHeight) {
		...
		if(curFrames == targetFrames) {
			curFrames = 0;
			curAnimation++;
			if(curAnimation == Spritesheet.mago_front.length) {
				curAnimation = 0;
			}
		}
...
public void render(Graphics g, int renderX, int renderY) {
		...
		if(down == true)
			g.drawImage(Spritesheet.mago_front[curAnimation],x - Camera.x, y - Camera.y,32,32,null);
		
		else if(right == true)
			g.drawImage(Spritesheet.mago_lado[curAnimation], x - Camera.x, y - Camera.y, 32, 32, null);
		
		else if(left == true)
			g.drawImage(Spritesheet.mago_lado[curAnimation], x + 32 - Camera.x, y - Camera.y, -32, 32, null);
		
		else if(up == true)
			g.drawImage(Spritesheet.mago_cima[curAnimation], x - Camera.x, y - Camera.y, 32, 32, null);
		
		else {
			g.drawImage(Spritesheet.mago_front[0], x - Camera.x, y - Camera.y, 32 , 32, null);
		}
