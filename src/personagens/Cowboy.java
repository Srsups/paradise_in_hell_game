...
public class Cowboy extends Rectangle{
	...
	public Cowboy(int x, int y,int screenWidth,int screenHeight) {
	    super(x, y, 32, 32);
...
public void render(Graphics g, int renderX, int renderY) {
		...
		if(down == true)
			g.drawImage(Spritesheet.cowboy_front[curAnimation],x - Camera.x, y - Camera.y,32,32,null);
		
		else if(right == true)
			g.drawImage(Spritesheet.cowboy_lado[curAnimation], x - Camera.x, y - Camera.y, 32, 32, null);
		
		else if(left == true)
			g.drawImage(Spritesheet.cowboy_lado[curAnimation], x + 32 - Camera.x, y - Camera.y, -32, 32, null);
		
		else if(up == true)
			g.drawImage(Spritesheet.cowboy_cima[curAnimation], x - Camera.x, y - Camera.y, 32, 32, null);
		
		else {
			g.drawImage(Spritesheet.cowboy_front[0], x - Camera.x, y - Camera.y, 32 , 32, null);
		}
