...
public class World2 {
	...
	private long sementeGlobal;
    private Random geradorGlobal;
	
	public World2() {
		
		// Use o horário do sistema como semente para garantir variação a cada execução
        sementeGlobal = System.currentTimeMillis();
        geradorGlobal = new Random(sementeGlobal);
	...
	public void render(Graphics g) {		
        ...
        for (int xBlock = xstart; xBlock <= xfinal; xBlock++) {
            for (int yBlock = ystart; yBlock <= yfinal; yBlock++) {
                ...
                Random rand = new Random((xBlock * 31) + yBlock + sementeGlobal);
                int num_rand = rand.nextInt(10000); // Use o valor desejado para o range

                BufferedImage spriteSelecionado = null;

                if (num_rand <= 3333) {
                    spriteSelecionado = Spritesheet.bg_World2[0];
                } else if (num_rand <= 6666) {
                    spriteSelecionado = Spritesheet.bg_World2[1];
                } else {
                    spriteSelecionado = Spritesheet.bg_World2[2];
                }
